...
export default function PropertyDetail({ property, error }) {
  ...
  // --- БЛОК БЕЗОПАСНОСТИ ---
  // Прерываем рендер, если данные отсутствуют (при холостой сборке Vercel).
  // Это абсолютно безопасно, так как все хуки выше уже объявлены.
  if (error || !property) {
    return (
      <>
        <Header setFilters={() => {}} />
  ...
  // Начинаются строго после проверки на существование property
