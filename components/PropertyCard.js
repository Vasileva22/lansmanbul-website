      ...
      <div className="img-container relative overflow-hidden h-[200px]">
        {photoUrls.length > 0 ? (
          <div className="w-full h-full relative group">
            ...
            {photoUrls.length > 1 && (
              <>
                <button onClick={handlePrev} className="slider-arrow arrow-left absolute left-2 top-1/2 -translate-y-1/2 bg-[#00a4a6] text-white rounded-full w-8 h-8 flex items-center justify-center text-sm opacity-80 hover:opacity-100 z-10">❮</button>
                <button onClick={handleNext} className="slider-arrow arrow-right absolute right-2 top-1/2 -translate-y-1/2 bg-[#00a4a6] text-white rounded-full w-8 h-8 flex items-center justify-center text-sm opacity-80 hover:opacity-100 z-10">❯</button>
              </>
            )}
          </div>
        ) : (
